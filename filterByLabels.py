...
with open("allowedLabels.txt") as file:
    allowed_labels = set(file.read().lower().split("\n"))

print(allowed_labels)

paths = list(Path("../filteredData").glob("./*/*.csv"))
...
for path in paths:
    new_path = Path("../filteredData2") / PurePath(*path.parts[2:])
...
filtered_paths = list(Path("../filteredData2").glob("./*/*.csv"))
